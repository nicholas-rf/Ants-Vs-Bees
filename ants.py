...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        # if insect.ant == QueenAnt:
        #     if insect.queenPlaced and not insect.imposterCreated:
        #         return
        #     if insect.imposterCreated:
        #         self.ant = None
            ...
            if insect.is_ant():
                if self.ant == insect:
                    if self.ant.container:
                        self.ant = self.ant.ant #if the ant is a container I.E bodyguard ant then it turns itself into the ant its contianing
                    elif self.ant.container and not self.ant.is_containing: 
                        self.ant = None #if the ant is a container but isnt containing anything
                    else:
                        self.ant = None
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        while place != hive:
            if place.bees and self.min_range <= rangeCheck <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            copiedBees = list(self.place.bees)
            damagedBees = [bee.reduce_armor(self.damage) for bee in copiedBees]
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        "*** YOUR CODE HERE ***"
        self.digesting = self.time_to_digest # sets digesting time to 3
        bee.reduce_armor(bee.armor) # kills bee
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        if QueenAnt.queenPlaced:
            self.imposterCreated = True
        Ant.__init__(self)
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        # if QueenAnt.queenLimit > 1:
        #     self.reduce_armor(self.armor)
        if self.imposterCreated:
            self.reduce_armor(self.armor)
            return
        else:
            ...
            while place:
                friend = place.ant
                if friend:
                    if friend.doubled != True:
                        friend.damage *= 2
                        friend.doubled = True
                    if friend.container:
                        friendInFriend = friend.ant
                        if friendInFriend.doubled != True:
                            friendInFriend.damage *= 2
                            friendInFriend.doubled = True
                place = place.exit
